...
새 기능·수정 사항을 버전별로 기록합니다. 업데이트 시 이 섹션에 항목을 추가합니다.
...
| V4.0 | 2026-02-09 | 노트 다중 카테고리 지원 | 노트가 여러 카테고리에 속할 수 있도록 `note_categories` 조인 테이블 기반 `category_ids` 구조를 대시보드·수업 자료·프로젝트·그래프·비교 뷰 전역에 반영. 서버에서 notes와 note_categories를 머지해 `category_ids` 배열을 만들고, 기존 `category_id` 값이 있을 때는 함께 포함되도록 호환 처리. |
| V4.0 | 2026-02-09 | 카테고리 사이드바 실시간 연동 | `categories` 테이블에 대한 Supabase Realtime 구독을 대시보드·수업 자료·프로젝트(목록/상세) 페이지에 추가. 한 화면에서 카테고리 이름·순서·삭제를 변경하면 다른 화면의 사이드바 카테고리도 자동으로 동기화되도록 개선. |
| V4.0 | 2026-02-09 | 수업 자료 링크 추가/수정 UX 개선 | 수업 자료 페이지에서 "링크 추가" 버튼이 링크 수정 중에도 항상 보이도록 변경. 수정 중에 "링크 추가"를 누르면 현재 수정 내용은 저장하지 않고 닫은 뒤, 바로 새 링크 추가 폼을 열도록 동작 통일. 사이드바에서 "수업 자료" 메뉴를 다시 눌렀을 때 링크 추가/수정 폼과 그룹·소그룹·링크 펼침 상태를 초기화해 첫 화면으로 돌아가도록 처리. |
| V4.0 | 2026-02-09 | 대시보드 노트 카드 액션 정렬 및 카테고리 이름 툴팁 | 노트 카드 우측의 즐겨찾기·카테고리·삭제 버튼을 한 줄로 정렬하고 아이콘 크기와 간격을 통일해 정돈된 UI로 개선. 노트에 적용된 카테고리 뱃지가 길어져 잘리는 경우, 마우스 오버 시 전체 카테고리 이름이 툴팁으로 보이도록 `title` 속성 추가. |
...
| V1.0 | 2026-02-09 | 대시보드 학습 통계 | 상단 4개 카드: 총 노트, 수업 자료 링크, 프로젝트, 공부 시간. 수업 자료·프로젝트 카드 클릭 시 해당 페이지 이동. 서버·포커스 시 개수 갱신. |
| V1.0 | 2026-02-09 | 공부 시간 | sessionStorage로 세션 시작 시각 저장. 다른 페이지 갔다 와도 같은 탭에서 누적. |
| V1.0 | 2026-02-09 | 그래프 뷰 터치 | 모바일에서 한 손가락 드래그로 그래프 패닝. touchstart/touchmove(passive:false)/touchend. 노드/링크 영역은 패닝 제외. |
| V1.0 | 2026-02-09 | README 전체 업데이트 | 업데이트 목차·사이트 개요·제공 기능·디렉터리 구조·라우팅·API 경로 등 모든 섹션 최신화. |
...
└── docs/                         # 설정·문제해결 가이드
    ...
    ├── 오류 해결 사항들/
    ├── 체크리스트/
    └── 업데이트 목차/             # 버전별 업데이트 요약 (README_(v2).md ~ README_(v5.0).md)
```
